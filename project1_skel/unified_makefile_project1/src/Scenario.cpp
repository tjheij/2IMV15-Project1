...
void scenarioConstraints(std::vector<Particle*> &particles, std::vector<Force*> &forces, std::vector<Constraint*> &constraints) {
	...
	constraints.push_back(new CircularWireConstraint(particles[0], circle_1_offset, 0.2f, 1000.0f, 1.0f));
	constraints.push_back(new CircularWireConstraint(particles[1], circle_2_offset, 0.1f, 1000.0f, 1.0f));
	constraints.push_back(new RodConstraint(particles[0], particles[2], 0.1f, 0.0f, 0.0f));
}
