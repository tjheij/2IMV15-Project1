...
void AngularSpringForce::apply_force() {
    ...
    double C = std::acos(cosTheta) - m_angle;
    ...
    Vec2f dCdx = -1 / std::sqrt(1 - cosTheta * cosTheta) * (dCdxT1 + dCdxT2);
    Vec2f dCdy = 1 / std::sqrt(1 - cosTheta * cosTheta) * (dCdxT1 + dCdzT1 + dCdxT2 + dCdzT2);
    Vec2f dCdz = -1 / std::sqrt(1 - cosTheta * cosTheta) * (dCdzT1 + dCdzT2);
